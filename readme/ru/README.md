<div align="center">
  ...
  <p>
    <h3>Масштабировать Контекст, Изучать Опыт</h3>
  </p>
  <p align="center">
...
Acontext — это платформа данных контекста, которая:

- **Хранит** контексты и артефакты
- **Наблюдает** за задачами агентов и обратной связью пользователей.
- Обеспечивает **самообучение** агентов путем сбора опыта (SOP) в долговременную память.
- Предлагает **локальную панель управления** для просмотра сообщений, задач, артефактов и опыта.
...
- **Создать более масштабируемый продукт агента**
...
- [**Session**](https://docs.acontext.io/store/messages/multi-provider) - Поток разговора, который хранит сообщения с поддержкой мультимодальности. 
  - [**Task Agent**](https://docs.acontext.io/observe/agent_tasks) - Фоновый TODO-агент, который собирает статус, прогресс и предпочтения задачи.
- [**Disk**](https://docs.acontext.io/store/disk) - Файловое хранилище для артефактов агентов.
- [**Space**](https://docs.acontext.io/learn/skill-space) - Похожее на Notion `Space` для агентов, где хранятся изученные навыки. 
  - [**Experience Agent**](https://docs.acontext.io/learn/advance/experience-agent) - Фоновые агенты, которые извлекают, сохраняют и ищут навыки.
...
Ваши навыки агента выглядят так:
...
Опыт агента будет храниться в структурированном `Space` с папками, страницами и блоками. Например:
...
>
> - `python/openai-agent-basic`: самообучающийся агент в openai agent sdk.
> - `python/agno-basic`: самообучающийся агент в agno framework.
> - `python/openai-agent-artifacts`: агент, который может редактировать и загружать артефакты.
...
>
> - `typescript/vercel-ai-basic`: самообучающийся агент в @vercel/ai-sdk
...
Acontext может управлять сессиями агентов и артефактами.
...
### Артефакты [📖](https://docs.acontext.io/store/disk)

Создайте диск для вашего агента для хранения и чтения артефактов, используя пути к файлам:
...
<div align="center">
  ...
  <p>Вы можете просматривать артефакты в локальной панели управления</p>
</div>
...
Для каждой сессии Acontext **автоматически** запустит фонового агента для отслеживания прогресса задачи и обратной связи пользователя. **Это как фоновый TODO-агент**. Acontext будет использовать его для наблюдения за ежедневной успешностью вашего агента.

Вы можете использовать SDK для получения текущего состояния сессии агента для Context Engineering, такого как сокращение и сжатие. 
