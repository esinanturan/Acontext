...
messages = [
    {
        "role": "system",
        "content": f"You are a helpful assistant with disk access.\n\n{context_prompt}",
    },
    {
        "role": "user",
...
const messages = [
  {
    role: 'system',
    content: `You are a helpful assistant with disk access.\n\n${contextPrompt}`,
  },
  {
    role: 'user',
...
<Step title="Include context prompt in system message">
  The context provides guidelines for using each tool. Include it in your system prompt.
  ...
  <Tip>
  The context prompt includes guidelines for file operations such as path formatting, read/write operations, and search capabilities.
  </Tip>
</Step>
