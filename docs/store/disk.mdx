...
The Disk API provides persistent, S3-backed storage for file artifacts in your Acontext projects. 

You can organize files using a filesystem-like structure with paths, store custom metadata, and generate secure download URLs.


<Tip>
Consider Acontext Disk as the google drive for your agent.
</Tip>

## Setup Filesystem for your Agent
<Card title="Agentic Disk SDK" icon="box" href="/tool/disk_tools">
Bring filesystem to your Agent with one-line code.
</Card>
If you like to know the details of Disk APIs, please read the following documentation.
...
<CodeGroup>
```python Python
# Create a new disk (optionally associate with a user)
disk = client.disks.create(user="user@example.com")
print(f"Created disk: {disk.id}")
```

```typescript TypeScript
// Create a new disk (optionally associate with a user)
const disk = await client.disks.create({ user: 'user@example.com' });
...
<Tip>
You can associate a disk with a specific user by passing the `user` parameter when creating it. 
This enables per-user resource management and cleanup. 
See [Per-user Resources](/chore/per-user) for details.
</Tip>
</Step>
...
<Step title="Search file contents with grep">
Search for text patterns within file contents using regex. This only searches text-based files (code, markdown, JSON, CSV, etc.).

<CodeGroup>
```python Python
# Search for files containing "authentication" in content
...
print(f"Found {len(results)} file(s) matching pattern:")
...
```typescript TypeScript
// Search for files containing "authentication" in content
const results = await client.disks.artifacts.grepArtifacts(disk.id, {
    query: "authentication",
    limit: 50
});
...
<Tip>
Use regex patterns like `"Goals.*"` to find sections, `"#.*Plan"` to find markdown headers, or `"TODO.*"` to find TODO comments. Remember: in regex, `.*` means "any characters" while `*` alone means "zero or more of the preceding character".
</Tip>
</Step>

<Step title="Find files with glob patterns">
...
print(f"Found {len(results)} markdown file(s):")
for artifact in results:
    print(f"  - {artifact.path}{artifact.filename}")

# Find files in documents/2024 folder
docs = client.disks.artifacts.glob_artifacts(
    disk.id,
    query="/documents/2024/*"
)
```
...
console.log(`Found ${results.length} markdown file(s):`);
results.forEach(artifact => {
    console.log(`  - ${artifact.path}${artifact.filename}`);
});

// Find files in documents/2024 folder
const docs = await client.disks.artifacts.globArtifacts(disk.id, {
    query: "/documents/2024/*"
});
```
</CodeGroup>
...
<Info>
Glob patterns are perfect for discovering files by extension or location without knowing exact filenames.
</Info>
</Step>
